...
Image models route through `image_model_providers`; video models route through `video_model_providers`. Each entry maps one `model` to one provider; supported providers are `ark` and `dashscope`. Requests whose model is not listed in the matching section are rejected.
